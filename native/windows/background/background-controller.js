...
export class BackgroundController {
  constructor() {
    ...
    this.valorantTracker = new ValorantRoundTracker();
  ...
  async run() {
    ...
    window.valorantRoundTracker = this.valorantTracker;

    console.log('✅ Valorant Tracker initialized:', window.valorantRoundTracker);
  ...
  _onInfoUpdate(infoUpdate) {
    try {
      // 1) keep Overwolf’s logic
      this.owInfoUpdatesStore.push(infoUpdate);
      this.owEventBus.trigger('info', infoUpdate);

      // 2) forward to our tracker
      if (window.valorantRoundTracker && infoUpdate && infoUpdate.info) {
        window.valorantRoundTracker._onInfoUpdate(infoUpdate.info);
      }
    } catch (e) {
      console.error("[BackgroundController] Failed forwarding infoUpdate", e);
    }
  }
}
